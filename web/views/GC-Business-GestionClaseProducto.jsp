<%-- 
    ...
    Sistema             : GC-Business
    Módulo              : Productos
    Nombre              : GC-Business-GestionClaseProducto.jsp
    Versión             : 1.0
    Fecha Creación      : 21-08-2018
    Autor Creación      : Pablo Jimenez Aguado
    Uso                 : Crear, Modificar, Consultar e Inactivar una clase de producto
--%>
...
<html>
    <head>
        ...
        <meta charset="utf-8" />
        <title>GC BUSINESS - Gestión de Clases de Producto</title>
    ...
    <body class="no-skin">
        ...
        <div class="main-container ace-save-state" id="main-container">
                    ...
                    <div class="page-content">
                        ...
                        <div class="page-header">
                            <h1>
                                ...
                                <small>
                                    ...
                                    Crear, modificar, eliminar clases de productos
                                </small>
                            </h1>
                        </div><!-- /.page-header -->
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                ...
                                <div>
                                    <table id="tablaClaseProductos" class="table table-striped table-bordered table-hover">
                                        <thead>
                                            <tr>
                                                <th>Nro</th>
                                                <th>Familia</th>   
            ...
            <!-- Modales -->
            <div class="modal" id="modalAgregarClaseProducto" tabindex="-1">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                            <h4 class="blue bigger">Registrar Nueva Clase Producto</h4>
                        </div>

                        <div class="modal-body">
                            <div class="row">
                                <div class="col-xs-12 col-sm-12">
                                    <input type="hidden" id="idclaseproducto" value="">
                                    <input type="hidden" id="opcion" value="">
                                    
                                    <div class="form-group">
                                        <label for="familiaproducto" class="col-sm-3 control-label">Familia</label>
                                        <div class="col-sm-9">
                                            <select id="familiaproducto" name="familiaproducto" class="col-xs-10 col-sm-6">
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                ...
                $('body').on('shown.bs.modal', '#modalAgregarClaseProducto', function () {
                    $('input:visible:enabled:first', this).focus();
                });            
            
                var tablaClaseProductos = $('#tablaClaseProductos').DataTable({
                    bAutoWidth: false,
                    "processing": true,
                ...
                new $.fn.dataTable.Buttons(tablaClaseProductos, {
                    buttons: [
                        {
                            ...
                            "action": function () {
                                $('#opcion').val('insertar');
                                $('#familiaproducto').val('0');
                                $('#descripcion').val('');
                                $('#abreviatura').val('');
                                $('#estado').val('A');
                                $('#divestado').hide();
                                $('#descripcion').prop('disabled', false);
                                $('#modalAgregarClaseProducto .blue').text('Registrar Nueva Clase Producto');
                                $('#modalAgregarClaseProducto').modal('show');
                                $('.divError').empty();
                            }
                        },
                        {
                ...
                tablaClaseProductos.buttons().container().appendTo($('.tableTools-container'));

                $('#btnGuardar').click(function (event) {
                    var idclaseproducto = $('#idclaseproducto').val();
                    ...
                    $.ajax({
                        method: "POST",
                        url: "../ClaseProducto",
                        data: {"opcion": opcion, "idfamiliaproducto": idfamiliaproducto, "idclaseproducto": idclaseproducto, "descripcion": descripcion, "abreviatura": abreviatura, "estado": estado}
                    }).done(function (data) {
                        ...
                        if (obj.mensaje.indexOf('ERROR') !== -1) {
                            ...
                            tablaClaseProductos.ajax.reload();
                            alertify.success(obj.mensaje);
                        }
                        $('#modalAgregarClaseProducto').modal('hide');
                    });
                });                         

                //Actualizar registro
                $(document).on('click', '.actualizar', function () {
                    var idclaseproducto = $(this).attr('id');
                    var row = $(this).parent().parent();
                    $.ajax({
                        url: "../ClaseProducto",
                        method: "POST",
                        data: {"opcion": "buscar", "idclaseproducto": idclaseproducto},
                        success: function (data) {
                            var obj = jQuery.parseJSON(data);
                            $('#opcion').val('actualizar');
                            $('#idclaseproducto').val(obj.idclaseproducto);
                            $('#familiaproducto').val(obj.idfamiliaproducto);
                            $('#descripcion').val(obj.descripcion);
                            $('#abreviatura').val(obj.abreviatura);
                            $('#estado').val(obj.estado);
                            $('#divestado').hide();
                            $('#modalAgregarClaseProducto .blue').text('Modificar Clase Producto');
                            $('#modalAgregarClaseProducto').modal('show');
                        },
                ...
                $(document).on('click', '.eliminar', function () {
                    ...
                    $.ajax({
                        url: "../ClaseProducto",
                        method: "POST",
                        data: {"opcion": "eliminar", "idclaseproducto": idclaseproducto},
                        success: function (data) {
                            var obj = jQuery.parseJSON(data);
                            if (obj.mensaje.indexOf('ERROR') !== -1) {
                                $('.divError').html(obj.html);
                ...
                $(document).on('click', '.activar', function () {
                    ...
                    $.ajax({
                        url: "../ClaseProducto",
                        method: "POST",
                        data: {"opcion": "activar", "idclaseproducto": idclaseproducto},
                        success: function (data) {
                            var obj = jQuery.parseJSON(data);
                            if (obj.mensaje.indexOf('ERROR') !== -1) {
                                $('.divError').html(obj.html);
